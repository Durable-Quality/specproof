...
### Changed
- The test-code panel's `SOURCE ·` file path now sits between the route description and the first test name, at the same size as the test name and in bold, instead of below the code blocks.
...
- The marketing footer's version tag was hardcoded and had gone stale at `v0.7.1`; `bun run generate:mockup` now rewrites it from `package.json`'s `version` on every run, so it can't drift again.

## [0.9.1] - 2026-09-04

### Changed
- Split this repo's own test suite into `tests/unit/` and `tests/integration/`, with `bun run test:unit` and `bun run test:integration` as separate scripts (`bun run test` still runs both). CI's `Test` job now runs them as two steps so a log or re-run can target one without pulling in the other.
- Trimmed `scripts/README.md` to cut redundant explanation while keeping every command and flag.
