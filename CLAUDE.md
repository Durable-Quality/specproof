...
- The spec and tests are **deliberately out of step** so the demo shows every verdict state: fully proven operations (`POST /auth/login`, `GET /tasks`, `GET /tasks/{taskId}`), partial coverage (`POST /tasks`, `PATCH /tasks/{taskId}`), an untested operation (`GET /projects`), and two statuses the tests assert but the spec omits (422 on `POST /tasks`, 500 on `DELETE /tasks/{taskId}`). No operation documents a 500, so every one of them carries a `MISSING FROM SPEC` row: six synthesized and stamped `NO TEST`, and `DELETE /tasks/{taskId}`'s, which the test asserts, stamped `UNDOCUMENTED`. That pair is what shows a status is never listed twice, and that the two stamps answer different questions. Preserve that mix when editing.
