...
`main` is protected by a repo ruleset (PR required, merge queue, no direct pushes). The flow: open a PR → the CI workflow (`.github/workflows/ci.yml`, jobs `Build` / `Lint` / `Test`) runs on the PR and again in the merge queue → all three checks must pass → clicking merge queues the PR and GitHub merges it (squash). The `Build` job also runs `bun run build:cli` and then smoke-tests `dist/scripts/cli.js` under a plain Node install with no Bun in the execution path — the regression guard for "the published CLI doesn't secretly require Bun."
